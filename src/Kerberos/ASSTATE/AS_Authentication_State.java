...
public class AS_Authentication_State extends AS_State {
    ...
    public void HandleAuthentication(NetworkPacket np) {
        ...
        try {            
            ...
            boolean isConnected=context.connectUser((String) np.get(KAS_CST.USERNAME), 
                    (String) np.get(KAS_CST.SALT), (String) np.get(KAS_CST.PWD));
            ...
            if(isConnected && tgsFound) {
                ...
                Ktgs=KeySerializator.getKey(
                        directory+(String) np.get(KAS_CST.USERNAME)+
                                context.getSERVER_EXT(), algorithm);
                ...
                TicketTGS ticketTGS=new TicketTGS((String)np.get(KAS_CST.USERNAME), 
                        (String) np.get(KAS_CST.INTERFACE),
                        LocalDate.now().plusDays(context.getValidite()), Kctgs); //définir la validité... 24h?
                ...
                if(!isConnected) { 
                    ...
                    System.err.printf("[KERBEROS AS]Username pas trouvé: %s : %s\n", 
                            (String)np.get(KAS_CST.USERNAME), 
        ...
        } catch (FileNotFoundException e){//clé d'utilisateur pas trouvé
